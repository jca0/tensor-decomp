...
from utils import reconstruct_tensor, reconstruct_tensor_tucker, fro_norm, factor_recovery_error, reconstruction_error
from tensor_decomp_algs import jennrich, cp_als, tucker_als
...
def experiment1(data_gen_func, d=20, k=3, sigma=0.1, num_reps=50):
    ...
    for rho in tqdm(rhos):
        ...
        for _ in tqdm(range(num_reps)):
            ...
            # Tucker
            A_hat, B_hat, C_hat, S = tucker_als(T_noisy, k)
            hat_T = reconstruct_tensor_tucker(A_hat, B_hat, C_hat, S)
            rec_t.append(reconstruction_error(T, hat_T))
            fac_t.append((factor_recovery_error(A_true, A_hat) + factor_recovery_error(B_true, B_hat) + factor_recovery_error(C_true, C_hat)) / 3)
        jenn_rec.append(np.mean(rec_j) if rec_j else np.nan)
        als_rec.append(np.mean(rec_a))
        tucker_rec.append(np.mean(rec_t))
        jenn_fac.append(np.mean(fac_j) if fac_j else np.nan)
        als_fac.append(np.mean(fac_a))
        tucker_fac.append(np.mean(fac_t))
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))

    ax1.plot(rhos, jenn_rec, label='Jennrich Rec Err')
    ax1.plot(rhos, als_rec, label='ALS Rec Err')
    ax1.plot(rhos, tucker_rec, label='Tucker Rec Err')
    ax1.legend()
    ax1.set_xlabel('rho')
    ...
    ax2.plot(rhos, tucker_fac, label='Tucker Fac Sim')
    ax2.legend()
    ax2.set_xlabel('rho')
...
def experiment2(data_gen_func, d=20, k=3, rho=0, num_reps=50):
    ...
    ax1.plot(sigmas, tucker_rec, label='Tucker Rec Err')
    ax1.legend()
    ax1.set_xscale('log')
    ...
    ax2.plot(sigmas, tucker_fac, label='Tucker Fac Sim')
    ax2.legend()
    ax2.set_xscale('log')
...
def experiment3(data_gen_func, d=20, k=3, sigma=0.1, rho=0.5, num_inits=20, num_reps=20):
    ...
    for _ in range(num_reps):
        ...
        for _ in range(num_inits):
            A_hat, B_hat, C_hat, S = tucker_als(T_noisy, k)
            hat_T = reconstruct_tensor_tucker(A_hat, B_hat, C_hat, S)
            rec_ts.append(reconstruction_error(T, hat_T))
            fac_ts.append((factor_recovery_error(A_true, A_hat) + factor_recovery_error(B_true, B_hat) + factor_recovery_error(C_true, C_hat)) / 3)
        var_t_rec.append(np.std(rec_ts))
        var_t_fac.append(np.std(fac_ts))
    print('Mean STD Jennrich Rec Err:', np.nanmean(var_j_rec))
    print('Mean STD ALS Rec Err:', np.nanmean(var_a_rec))
    print('Mean STD Tucker Rec Err:', np.nanmean(var_t_rec))
    print('Mean STD Jennrich Fac Sim:', np.nanmean(var_j_fac))
    print('Mean STD ALS Fac Sim:', np.nanmean(var_a_fac))
    print('Mean STD Tucker Fac Sim:', np.nanmean(var_t_fac))